...
(function() {
	WS.dataModel = {
		create: function (appConfig) {
			...
			var defaultRenderer = function() { return true; };
			...
			var bindElements = function (elements) {
				WS.each(elements, function (index, element) {
					...
					if (WS.isNode(element) && element.type !== 'radio' && (element.classList.contains('ui-autocomplete-input') || element.classList.contains('stv_autocomplete'))) {
					...
					if (isSubtableElement(element)) {
						...
						if(appRenderers[path[0]] && appRenderers[path[0]][path[2]] && typeof appRenderers[path[0]][path[2]] === 'function') {
							elementConfig.render = appRenderers[path[0]][path[2]];
							elementConfig.hasCustomRenderer = true;
						}

						if(appListeners[path[0]] && appListeners[path[0]][path[2]] && typeof appListeners[path[0]][path[2]] === 'function') {
						...
						WS.each(listenerTypes, function() {
							...
							document.addEventListener(type, function (evt) {
								...
								if(isSubtableElement(evt.target)) {
									...
									if(appListeners[path[0]] && appListeners[path[0]][path[2]]
									...
									if(appListeners['global'] && appListeners['global'][type] && typeof appListeners['global'][type] === 'function') {
								...
								} else  if (model[evt.target.id]
									&& (appListeners[evt.target.id] || appListeners['global'])) {

									if(appListeners[evt.target.id] && typeof appListeners[evt.target.id][type] === 'function') {
			...
			var registerLeaf = function (object, property, elementConfig) {
				if (WS.isNode(elementConfig.element)) {
					if (elementConfig.element.classList.contains('jr-dialog-form-table-number') || elementConfig.element.classList.contains('decimal') ) {
						registerDecimalLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('textarea') || elementConfig.element.classList.contains('stv_textarea')
						|| elementConfig.element.classList.contains('textbox') || elementConfig.element.classList.contains('stv_textbox')
						|| elementConfig.element.classList.contains('password') || elementConfig.element.classList.contains('jr-dialog-form-table-add-rows-count')) {
						registerDefaultInputLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('jr-checkbox') || elementConfig.element.classList.contains('stv_checkbox')) {
						registerCheckboxLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('jr-btn') || elementConfig.element.classList.contains('stv_button')) {
						registerButtonLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('jr-dialog-form-date') || elementConfig.element.classList.contains('stv_date') || elementConfig.element.classList.contains('stv_datetime')) {
						registerDateLeaf(object, property, elementConfig);
						hookIntoJRsBrokenDateImplementation = true;
					} else if (elementConfig.element.classList.contains('file') || elementConfig.element.classList.contains('stv_file')) {
						registerFileLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('ui-autocomplete-input') || elementConfig.element.classList.contains('stv_autocomplete')) {
						registerAutocompleteLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('list') || elementConfig.element.classList.contains('stv_list')
						|| elementConfig.element.classList.contains('users') || elementConfig.element.classList.contains('stv_users')) {
						registerListLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('sqlList') || elementConfig.element.classList.contains('stv_sqllist')) {
						registerSqlListLeaf(object, property, elementConfig);
					} else if (elementConfig.element.classList.contains('sqlTextbox') || elementConfig.element.classList.contains('stv_sqltextbox')) {
					...
					} else if (elementConfig.element.classList.contains('text') || elementConfig.element.classList.contains('stv_text') || elementConfig.element.classList.contains('stv_image')) {
			...
			var registerDecimalLeaf = function (object, property, elementConfig) {
				...
				if(elementConfig.isSubtableElement) {
					elementLabel = document.getElementById(retrieveFromSTVHeaderElement(element.id)).querySelector('span');
				} else {
			...
			var registerDateLeaf = function (object, property, elementConfig) {
				...
				Object.defineProperty(leaf, 'value', {
					...
					set: function (newDate) {
						...
						if((leaf.value === '' && newDate instanceof Date) || (newDate instanceof Date && newDate.getTime() !== leaf.value.getTime())) {
			...
			var registerRadioLeaf = function (object, property, elementConfig) {
				...
				WS.each(elements, function (index) {
					...
					if(elementConfig.isSubtableElement) {
						elementLabel = document.getElementById(retrieveFromSTVHeaderElement(elements[index].id)).querySelector('span');
					} else {	
				...
				if(appListeners[property]) {
					WS.each(appListeners[property], function(type) {
						...
						elementContainer.addEventListener(type, function(evt) {
							appListeners[property][type].apply(evt.srcElement.innerHTML, [leaf[evt.srcElement.parentNode.rowIndex - 1]]);
						});
			...
			var registerFileLeaf = function (object, property, elementConfig) {
				...
				if(elementConfig.isSubtableElement) {
					...
					elementLabel = document.getElementById(retrieveFromSTVHeaderElement(elementId)).querySelector('span');
					elementContainer = document.getElementById('div_' + elementId).querySelector('.jr-file-upload-controls');
				} else {
			...
			var renderLeaf = function (leaf, type, callback) {
				...
				var invokeDependency = function(dependendLeaf) {
					...
					if(sqlElements.indexOf(dependendLeaf._type) > -1 &&
					(refreshTypes.indexOf(type) > -1 || (type === 'dependency' && refreshTypes.indexOf(initialType) > -1))) {
			...
			var addSubtableFunctions = function () {
				WS.each(WS.get('.subtableView'), function () {
					...
					jrSubtable.afterAddRowCallbacks.push(function (jrSubtableId, insertedRow) {
						...
						WS.each(row, function() { // renderer execution has to happen after whole row has been initialized
			...
			var bindContainerElements = function() {
				...
				WS.each(WS.get('#pageContainer .row, #pageContainer .col'), function() {
					...
					Object.defineProperty(model, this.id, {
						...
						configurable: true,
						set: function () {
							console.log('It is not allowed to overwrite the "' + this.id + '" property of the model object.');
						},
						get: function () {
							return leaf;
						},
									...
									if(snapShot1Rows[rowId] && snapShot2Rows[rowId]) {
										WS.each(snapShot1Rows[rowId], function(column) {
											if(this != snapShot2Rows[rowId][column]) {
												addSubtableDiv(elementName, rowId, column, this, snapShot2Rows[rowId][column]);
											}
										});
